-- Increment current_level when a case is completed successfully.
-- This fixes progression where users couldn't reach the next phase after finishing the first one.
...
BEGIN
  ...
  IF _is_correct THEN
    ...
    IF NOT _already_completed AND _time_taken IS NOT NULL THEN
      ...
      -- Update profile stats AND progression level
...
GRANT EXECUTE ON FUNCTION public.verify_case_solution(UUID, JSONB, INTEGER) TO authenticated;
